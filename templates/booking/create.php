    ...
    <div class="counselor-detail" style="max-width:640px;">
        ...
        <?php else: ?>
            <form method="post" action="/bookings">
                ...
                <div class="mb-3">
                    <label for="schedule_id" class="form-label">Tanggal &amp; Jam</label>
                    <select id="schedule_id" name="schedule_id" class="form-select" required>
                        <option value="">Pilih tanggal tersedia</option>
                        <?php foreach ($slots as $slot): ?>
                            <option value="<?= (int) $slot['schedule_id'] ?>" <?= (int) ($old['schedule_id'] ?? 0) === (int) $slot['schedule_id'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($slot['date'] ? date('d M Y', strtotime($slot['date'])) : '-') ?>, <?= htmlspecialchars(substr($slot['start_time'], 0, 5)) ?>-<?= htmlspecialchars(substr($slot['end_time'], 0, 5)) ?> (sisa <?= (int) $slot['remaining_quota'] ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    <label for="complaint" class="form-label">Keluhan (opsional)</label>
                    <textarea id="complaint" name="complaint" class="form-control" rows="3"><?= htmlspecialchars($old['complaint'] ?? '') ?></textarea>
                </div>
